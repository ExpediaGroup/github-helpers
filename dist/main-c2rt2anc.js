...
var require_hasown2 = __commonJS(function(exports, module) {
  var call = Function.prototype.call;
  var $hasOwn = Object.prototype.hasOwnProperty;
  var bind = require_function_bind();
  module.exports = bind.call(call, $hasOwn);
...
var require_hasown3 = __commonJS(function(exports, module) {
  var call = Function.prototype.call;
  var $hasOwn = Object.prototype.hasOwnProperty;
  var bind = require_function_bind();
  module.exports = bind.call(call, $hasOwn);
    ...
    function transformRequest(data, headers) {
      ...
      if (isObjectPayload) {
        ...
        if ((isFileList = utils_default.isFileList(data)) || contentType.indexOf("multipart/form-data") > -1) {
          ...
          return toFormData_default(isFileList ? { "files[]": data } : data, _FormData && new _FormData, formSerializer);
        }
      }
...
var http_default = isHttpAdapterSupported && function httpAdapter(config) {
  return wrapAsync(async function dispatchHttpRequest(resolve, reject, onDone) {
    ...
    onDone((response, isRejected) => {
      ...
      const { data } = response;
      if (data instanceof stream3.Readable || data instanceof stream3.Duplex) {
        const offListeners = stream3.finished(data, () => {
          offListeners();
          onFinished();
        });
      } else {
...
function resolveConfig(config) {
  ...
  const own = (key) => utils_default.hasOwnProp(newConfig, key) ? newConfig[key] : undefined;
  const data = own("data");
...
var factory = (env) => {
  ...
  return async (config) => {
    ...
    const own = (key) => utils_default.hasOwnProp(config, key) ? config[key] : undefined;
...
var notifyUser = async ({ login, pull_number, slack_webhook_url }) => {
  const email = await getEmailOnUserProfile2({ login });
  if (!email) {
    return;
  }
  const {
  ...
  info(`Notifying user ${login}...`);
  const result = await axios_default.post(slack_webhook_url, {
    assignee: email,
    title,
    html_url,
  ...
  if (result.status !== 200) {
    ...
    setFailed(`User notification failed for login: ${login} and email: ${email}`);
  }
  return result;
};
...
//# debugId=24EBDD776807F9CF64756E2164756E21
